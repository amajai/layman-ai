      ...
      <ul className='h-4/6 overflow-y-auto'>
        {data.length > 0 ?
          (data.map((prompt) => (
            <li
              key={prompt.id}
              className={`mb-2 p-2 rounded flex justify-between items-center shadow cursor-pointer promptItem ${selectedPromptId == prompt.id ? 'bg-blue-400 text-white' : 'bg-white'}`}
